...
как `"configure"`/`"start"`. Mesh-сигналинг (`call_join`/`call_leave`/
`call_signal`) при этом продолжает ходить — chat-service всё ещё
использует `call_join` для назначения SFU-комнаты, — но
`onCallRosterReceived()` больше не создаёт mesh-`PeerConnection`; сам
mesh-код остаётся в файле неиспользуемым до #233. Известный пробел:
трек, добавленный к уже согласованному publish-соединению
(`enableVideo()`/`enableScreenShare()` посреди звонка), не долетает до
уже подписавшихся участников — Janus не проталкивает новый поток
существующим подписчикам сам, а повторная подписка уже открытого
subscribe-handle на новый поток пока не реализована.
