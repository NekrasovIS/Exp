...
apps/
  web/       React-клиент (issue #220, зонтичная задача — #264-#269):
             каркас/сессия/роутинг (#264), вход/регистрация/OTP (#265),
             сообщества/каналы (#266), чат канала (#267), друзья/личные
             диалоги (#268) готовы; шифрование каналов (#269) отложено
...
делает `MainWindow.cpp`). Зашифрованные каналы (issue #136/#138) в
веб-клиенте пока не расшифровываются (`ChatView` показывает заглушку
и не открывает подписку/не грузит историю для такого канала вообще,
issue #269 — отдельная задача).
