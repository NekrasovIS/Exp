...
- Личные сообщения и друзья (issue #187). **Фаза 1** — заявки в друзья:
  ...
  чата канала. Backend Фазы 2 пока не доставляет сообщения через
  WebSocket, поэтому открытый диалог опрашивается раз в 4 секунды
  (`kDmPollIntervalMs`) вместо живой доставки — Фаза 2b добавит её
  отдельной задачей.
