...
градиентом, `src/ui/Theme.h`): слева сайдбар из двух колонок — узкая иконочная
полоса сообществ (72px, круглые аватарки с первой буквой названия; «+»
снизу создаёт новое, правый клик — войти/переименовать/удалить) и
список каналов выбранного сообщества (240px, «+» создаёт канал, правый
клик — переименовать/удалить, названия с префиксом «#»); клик по
каналу открывает чат в центре. Справа от чата — список участников
выбранного сообщества (`MemberListPanel`, 240px; сворачивается и
разворачивается иконкой в правом углу шапки канала, issue #184).
Сообщение
отправляется через единый скруглённый композер (иконка «+» слева для
вложений, поле ввода без своей рамки, зелёная стрелка справа вместо
подписанной кнопки «Send»). Футер снизу — профиль (клик по аватару
открывает меню Edit Profile/Sign Out) и настройки устройств (Audio
Output, Microphone, Camera, Screen Capture) в отдельном диалоге.
