...
(`MemberListPanel`, 240px). В режиме друзей та же вторая колонка
показывает список друзей и входящих заявок вместо каналов, а основная
область — упрощённый диалог личных сообщений вместо чата канала.
Сообщение отправляется через единый скруглённый композер (иконка «+»
слева для вложений, поле ввода без своей рамки, зелёная стрелка справа
вместо подписанной кнопки «Send»). Футер снизу — профиль (клик по
аватару открывает меню Edit Profile/Sign Out) и настройки устройств
(Audio Output, Microphone, Camera, Screen Capture) в отдельном диалоге.
