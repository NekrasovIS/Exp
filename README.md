...
реальном времени, пока клиент подключён. `{"edit_message": {"id",
"body"}}`/`{"delete_message": {"id"}}` (issue #107) — редактировать
или удалить может только автор сообщения, не владелец канала/сообщества
(в отличие от переименования/удаления самих каналов и сообществ) —
эта модерационная возможность над чужими сообщениями сознательно
оставлена за рамками задачи, кандидат для будущей роли/модерации.
Успех рассылается всем подписчикам как `{"message_edited": {"id",
"body", "edited_at"}}`/`{"message_deleted": {"id"}}`; 404/403 уходят
только инициатору.
