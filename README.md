...
(кнопка «Friends» сверху — issue #187, Фаза 3, переключает в режим
друзей/личных сообщений, см. ниже; круглые аватарки — первая буква
названия, только те, в которых уже состоит вошедший пользователь; «+»
снизу открывает окно подключения — вступить по коду приглашения или
создать новое, — правый клик по аватарке копирует код приглашения, для
владельца ещё и перевыпускает его/переименовывает/удаляет) и список
...
пространстве окна. В режиме друзей та же вторая колонка показывает
список друзей и входящих заявок вместо каналов, а основная область —
упрощённый диалог личных сообщений вместо чата канала. Кнопка аккаунта
в правом верхнем углу, футер с профилем и настройками устройств (Audio
Output, Microphone, Camera, Screen Capture) слева. Тёмная тема с
зелёным градиентным акцентом (`src/ui/Theme.h`).
