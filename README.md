...
названия; «+» снизу создаёт новое, правый клик по аватарке —
войти/переименовать/удалить) и список каналов выбранного сообщества
рядом с ней («+» создаёт канал, правый клик — переименовать/удалить);
клик по каналу открывает чат в основном пространстве окна. В режиме
друзей та же вторая колонка показывает список друзей и входящих заявок
вместо каналов, а основная область — упрощённый диалог личных
сообщений вместо чата канала. Кнопка аккаунта в правом верхнем углу,
футер с профилем и настройками устройств (Audio Output, Microphone,
Camera, Screen Capture) слева. Тёмная тема с зелёным градиентным
акцентом (`src/ui/Theme.h`).
