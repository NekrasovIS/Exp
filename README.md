...
- Вход по одноразовому коду через email (issue #156) — окно `LoginWindow`
  показывается при каждом запуске (токен между перезапусками пока не
  сохраняется): ввод логина/email → код отправляется на email аккаунта →
  ввод кода завершает вход. Требует, чтобы у аккаунта уже был задан
  email (`Account` → `Edit Profile`, поле пока не выведено в UI отдельно
  — задаётся через тот же PATCH, что и остальной профиль); своей
