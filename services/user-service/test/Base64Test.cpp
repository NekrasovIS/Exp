...
TEST(Base64Test, EncodesRfc4648TestVectors) {
    EXPECT_EQ(encode(""), "");
...
TEST(Base64Test, EncodeThenDecodeRoundTripsArbitraryBytes) {
    const std::string original("\x00\x01\x02\xff\x7f\x80some text too", 17);
...
// Тот же приём временной замены fuzzing'у, что и в
// chat-service/test/Base64Test.cpp (issue #121) — контролируемый
// атакующим `data_base64` из POST /profile/avatar (issue #384) это
// такой же недоверенный ввод. Единственное проверяемое свойство —
// что decode() никогда не падает и не приводит к UB на произвольных
// байтах, включая встроенный NUL и весь диапазон 0-255.
