 ...
 * первого потребителя).
 *
 * Issue #227 (покрытие тестами) — не покрыто юнит-тестами и осознанно
 * оставлено так: требует настоящего TLS-соединения с реальным сервером
 * на другом конце (SMTP или Telegram Bot API), а не то, что можно
 * подменить моком на уровне этого класса — единственные потребители
 * (SmtpCodeDeliveryChannel::send(), TelegramCodeDeliveryChannel::send())
 * несут ту же самую оговорку.
 */
class TlsConnection {
public:
