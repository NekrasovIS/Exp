 ...
 *
 * Ключ, которым HttpServer вызывает allow() — httplib::Request::
 * remote_addr, реальный адрес TCP-пира с сокета, а не значение
 * какого-либо заголовка (issue #224, pentest): подделать `X-Forwarded-
 * For` для обхода лимита нельзя, поскольку он никак не используется.
 * Это же значит, что за реальным reverse-proxy (который подставлял бы
 * собственный адрес прокси для всех клиентов) лимитер нужно будет
 * научить читать доверенный заголовок — сейчас, без прокси перед
 * сервисом, читать remote_addr напрямую строго безопаснее.
 *
 * Ещё один компромисс первого прохода (issue #224, pentest): ключ —
 * сырая строка адреса без нормализации, так что один и тот же клиент,
 * подключившийся через разные представления одного адреса (например,
 * IPv4 `127.0.0.1` и IPv4-mapped IPv6 `::ffff:127.0.0.1` на
 * dual-stack listener), получает независимые окна и тем самым может
 * удвоить свой лимит. Не эксплуатируется в текущем деплойменте —
 * AUTH_SERVICE_HOST по умолчанию `127.0.0.1`, единственный
 * IPv4-сокет, не dual-stack — но стоит учитывать при смене host на
 * `::`/dual-stack адрес или при добавлении reverse-proxy.
 */
class RateLimiter {
public:
