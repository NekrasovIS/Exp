...
TEST(HttpServerTest, RefreshRouteRejectsMissingFieldsWith400) {
    const TokenService tokenService("test-secret");
    const UserServiceClient userServiceClient("127.0.0.1", 1);
    const ScopedServer server(tokenService, userServiceClient);
...
TEST(HttpServerTest, RefreshRouteRejectsInvalidRefreshTokenWith401) {
    const TokenService tokenService("test-secret");
    const UserServiceClient userServiceClient("127.0.0.1", 1);
    const ScopedServer server(tokenService, userServiceClient);
    ...
    httplib::Client client(kTestHost, kTestPort);
    const httplib::Result result = client.Post(
        "/auth/refresh", nlohmann::json{{"refresh_token", "not-a-real-token"}}.dump(), "application/json");
...
TEST(HttpServerTest, RefreshRouteRejectsAnAccessTokenPresentedAsARefreshTokenWith401) {
    // issue #224 (pentest): un access-токен не должен работать вместо
    // refresh-токена, даже несмотря на общий секрет — TokenService уже
    // проверяет это через isRefresh/expectRefresh (см.
    // TokenServiceTest.cpp), здесь — та же проверка через реальный HTTP-маршрут.
    const TokenService tokenService("test-secret");
    const UserServiceClient userServiceClient("127.0.0.1", 1);
    const Token accessToken = tokenService.issueToken("alice");
    const ScopedServer server(tokenService, userServiceClient);
...
TEST(HttpServerTest, RefreshRouteIssuesFreshAccessTokenForValidRefreshToken) {
    const TokenService tokenService("test-secret");
    const UserServiceClient userServiceClient("127.0.0.1", 1);
    const Token refreshToken = tokenService.issueRefreshToken("alice");
    const ScopedServer server(tokenService, userServiceClient);
    ...
    httplib::Client client(kTestHost, kTestPort);
    const httplib::Result result = client.Post(
        "/auth/refresh", nlohmann::json{{"refresh_token", refreshToken.value}}.dump(), "application/json");
    ...
    EXPECT_FALSE(body["token"].get<std::string>().empty());
    EXPECT_GT(body["expires_at"].get<std::int64_t>(), 0);
    EXPECT_TRUE(tokenService.verifyToken(body["token"].get<std::string>()).has_value());
}

TEST(HttpServerTest, RefreshRouteIsNotRateLimitedByDesign) {
    // issue #224 (pentest): проверено осознанно, а не забыто —
    // /auth/refresh не проходит через RateLimiter (в отличие от
    // /auth/token, /auth/register, /auth/otp/*), см. doc-комментарий
    // HttpServer::HttpServer() и README, раздел auth-service: тот же
    // аргумент, что и у /auth/verify — не подбирается перебором, нужен
    // уже валидно подписанный токен, а не пароль/код. Этот тест
    // фиксирует поведение явно, чтобы будущее изменение было осознанным
    // решением, а не случайной регрессией в любую сторону.
    const TokenService tokenService("test-secret");
    const UserServiceClient userServiceClient("127.0.0.1", 1);
    const ScopedServer server(tokenService, userServiceClient);

    httplib::Client client(kTestHost, kTestPort);
    const nlohmann::json requestBody{{"refresh_token", "not-a-real-token"}};
    // HttpServer's default rate limit — 10 requests per window (см.
    // HttpServer.h) — все запросы сверх этого числа всё равно должны
    // получать 401 (invalid token), а не 429.
    for (int i = 0; i < 11; ++i) {
        const httplib::Result result = client.Post("/auth/refresh", requestBody.dump(), "application/json");
        ASSERT_TRUE(result);
        EXPECT_EQ(result->status, 401);
    }
}
