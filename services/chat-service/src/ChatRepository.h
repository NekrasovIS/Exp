...
public:
    ...
    /// стороне клиента; этот вызов никогда не видит сырой ключ.
    /// Полномочия (issue #217): собственный владелец канала, владелец
    /// родительского сообщества, его модератор — или любой другой
    /// участник, у которого в channel_keys уже есть собственная
    /// обёрнутая копия этого ключа. Только тот, кто уже владеет сырым
    /// ключом (создатель, или кто-то, для кого он уже был развёрнут), в
    /// принципе мог изготовить валидную обёрнутую копию для кого-то ещё
    /// — поэтому проверка "уже есть ключ" не ослабляет защиту, а лишь
    /// снимает искусственное ограничение только владельцем/модератором,
    /// позволяя любому участнику зашифрованного канала довыдать доступ
    /// новому участнику самостоятельно.
