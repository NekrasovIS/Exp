...
std::int64_t ChatRepository::markChannelRead(std::int64_t channelId, const std::string& login,
    ...
    // RETURNING (issue #380) hands back the value that actually ended up
    // in the column, so the caller can tell a genuine advance (result ==
    // messageId) from a stale/duplicate call (result > messageId) and
    // only broadcast a read receipt in the former case.
...
std::vector<ReadReceipt> ChatRepository::listChannelReadState(std::int64_t channelId) {
    ...
    receipts.reserve(static_cast<std::size_t>(rows.size()));
    for (const auto& row : rows) {
        receipts.push_back(
            ReadReceipt{.login = row[0].as<std::string>(), .lastReadMessageId = row[1].as<std::int64_t>()});
    }
    return receipts;
}
