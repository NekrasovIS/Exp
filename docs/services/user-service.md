...
Загрузка аватара (issue #384) — `POST /profile/avatar`
(`Authorization: Bearer` обязателен) принимает
`{"content_type", "data_base64"}` (та же base64-в-TEXT схема, что и у
вложений chat-service, issue #116, не object storage), проверяет, что
`content_type` начинается с `image/` и декодированный размер не
превышает 2 МБ (аватар — маленькая иконка, а не файл-вложение;
отдельный, более строгий лимит от 5 МБ chat-service), сохраняет байты
в `user_avatars` (одна строка на пользователя — новая загрузка
полностью заменяет предыдущую) и сразу обновляет `avatar_url` самого
профиля на `/users/<login>/avatar`, так что `GET .../profile`
отражает новый аватар без отдельного `PATCH`. Отдаёт его же
`GET /users/{login}/avatar` — единственный эндпоинт этого сервиса
**без** авторизации, кроме двух межсервисных выше: `<img src="...">`
браузер запрашивает напрямую по атрибуту, не может приложить заголовок
`Authorization`, а аватар и так публичен для любого, кто знает логин
(тот же уровень доверия, что и у `public_key`).
