...
`GET /internal/totp-status?login=` (issue #389) — тем же паттерном, что
`GET /internal/friendship`: без авторизации, только для auth-service,
чтобы решить, спрашивать ли TOTP-код при входе, ещё до выдачи какого бы
то ни было токена; отвечает `{"enabled"}`.
