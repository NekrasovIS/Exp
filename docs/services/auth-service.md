...
Вход при включённой двухфакторной аутентификации (issue #389, второй
шаг после #388 — TOTP на стороне user-service): если первичный фактор
успешен (пароль в `/auth/token` или одноразовый код в
`/auth/otp/verify`), но у аккаунта включена TOTP
(`GET /internal/totp-status` у user-service), ответ вместо обычной
пары токенов — `{"totp_required": true, "pending_token": ...}`.
`pending_token` — недолговечный (5 минут по умолчанию) отдельный вид
токена (`TokenService` подписывает его с `"typ": "totp-pending"`), не
принимаемый `/auth/verify`/`/auth/refresh` как обычный access/refresh
токен. Он обменивается на настоящую пару токен/refresh-токен через
`POST /auth/totp/verify` (`{"pending_token", "code"}` — `code`
принимается и как TOTP-код из приложения-аутентификатора, и как
резервный код, issue #388, проверяется у user-service через
`POST /users/verify-totp`). `/auth/totp/verify`, как и остальные
маршруты, проверяющие код/учётные данные, ограничен по частоте на
клиентский адрес. Аккаунты без включённой TOTP по-прежнему логинятся
за один запрос — `/auth/register` тоже не проходит через эту проверку,
у только что созданного аккаунта TOTP не может быть включена.
