 ...
 * сообщений.
 *
 * sendPinMessage()/sendUnpinMessage() (issue #338) отправляют
 * `{"pin_message": {"id"}}`/`{"unpin_message": {"id"}}` — правило
 * полномочий СТРОЖЕ, чем у edit/delete: только владелец канала/
 * сообщества или модератор, никогда сам автор сообщения как таковой
 * (закрепление — функция управления каналом, не модерация конкретного
 * сообщения). При успехе рассылка всем подписчикам через
 * messagePinned()/messageUnpinned() — тот же паттерн, что и выше.
 *
...
public:
    ...
    /// Закрепляет сообщение @p id в подписанном канале (issue #338) —
    /// доступно только владельцу канала/сообщества или модератору
    /// сообщества (сервер отклонит запрос иначе); идемпотентно.
    void sendPinMessage(qint64 id);

    /// Снимает закрепление сообщения @p id — то же правило полномочий,
    /// что и у sendPinMessage(); идемпотентно.
    void sendUnpinMessage(qint64 id);
...
signals:
    ...
    /// Сообщение @p id было закреплено (issue #338) — @p pinnedBy/@p
    /// pinnedAt относятся к ПЕРВОНАЧАЛЬНОМУ закреплению, даже если этот
    /// конкретный вызов sendPinMessage() был повторным (идемпотентно,
    /// см. doc-комментарий chat-service::PinMessageResult).
    void messagePinned(qint64 id, const QString& pinnedBy, const QString& pinnedAt);

    /// Закрепление сообщения @p id было снято.
    void messageUnpinned(qint64 id);
