...
void ChannelsPanel::rebuildList() {
    ...
    for (const ChatItem& channel : sorted) {
        ...
        // каналы получают ещё и значок замка, тот же, что и заголовок
        // ChatView (issue #138), прямо в строке списка, а не только
        // после открытия канала (issue #152) — оба только в text(),
