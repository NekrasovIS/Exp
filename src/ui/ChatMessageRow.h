...
signals:
    /// Клик по "Edit" (только для собственных сообщений, issue #107) —
    /// @p currentBody позволяет вызывающему коду заранее заполнить поле
    /// редактирования, не разыскивая сообщение по id.
    void editRequested(qint64 id, const QString& currentBody);

    /// Клик по "Delete" (только для собственных сообщений).
